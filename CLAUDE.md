...
- Python 3.11+, Streamlit, Qdrant (Docker, external), SQLite, Pytest
- LLM/Embeddings via OpenRouter API: chat model `minimax/minimax-m3:free`
- Embeddings (local, free): `sentence-transformers all-MiniLM-L6-v2` (384-dim, cosine)
- Docker + docker-compose for app shipping (Qdrant stays external, NOT in compose)
...
uv run python src/ingest.py              # CSV -> SQLite + policy chunks -> Qdrant
uv run streamlit run src/app.py          # chat UI + telemetry dashboard

uv run pytest -q                # unit + retrieval tests
docker-compose up --build         # app container (expects Qdrant reachable at QDRANT_HOST + OPENROUTER_API_KEY set)
```
